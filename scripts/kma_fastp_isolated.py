...
def bsub_submit(command='', directory='', modules='', runtime=60, cores=1, hosts=1, ram=1, queue='hpc', name='',
    ...
    # The submit
    # return script
    file_name = f'{directory}/{script_file_id}_temp.pbs.sh'
...
if __name__ == "__main__":
    ...
    ram_list = [12, 40, 100]
    minutes_list = [120, 240, 600]
    #module_list = "tools anaconda/2021.05 spades/3.15.5"
    data_directory = "/zhome/af/3/181344/main_folder/data/raw"
    time_output_directory = "/zhome/af/3/181344/main_folder/data/experiment"
    for file_id, ram_amount, minutes in zip(id_list, ram_list, minutes_list):
        ...
        # record file size
        program_command += f"touch {time_output_directory}/fastp_performance_{file_id}.txt" + "\n"
        ...
        program_command += f"perf stat -o {time_output_directory}/fastp_performance_{file_id}.txt -r 50 fastp -w 1 -i /dev/shm/{file_id}.fastq -o /dev/null" + "\n"
