...
#BSUB -oo blend_pipe.out 
#BSUB -eo blend_pipe.err 
### -- Specify CPU model
...
for i in 1 2 4 8
do 
    ecoli_time_file=~/main_folder/data/experiment_raw/gpu_times/blend_ecoli_pipe_times_$i.txt

    touch $ecoli_time_file

    for j in {1..5}
    do
        fastp -w 8 -i ~/main_folder/data/raw/SRR15334628_1.fastq.gz -I ~/main_folder/data/raw/SRR15334628_2.fastq.gz --length_required 200 -b 200 -B 200 --stdout | /usr/bin/time -v -o $ecoli_time_file -a blend -ax sr -t $i ~/main_folder/databases/ecoli_genome/GCA_018185575.1.fasta /dev/stdin | samtools view -h -F 4 /dev/stdin > /dev/shm/ecoli_blend_pipe.sam
        rm /dev/shm/ecoli_blend_pipe.sam
        sleep 5 
    done
done
