...
sed -i 's/CFLAGS += -std=c99/CFLAGS += -std=c99\nLDFLAGS ?= -flto/g' Makefile
sed -i 's/$(CC) $(CFLAGS) -c -o $@ $</$(CC) -c $(CFLAGS) $(LDFLAGS) -o $@ $</g' Makefile
sed -i 's/CFLAGS += -std=c99/CC=\/appl\/gcc\/13.1.0-binutils-2.40\/bin\/gcc\nAR=\/appl\/gcc\/13.1.0-binutils-2.40\/bin\/gcc-ar/g' Makefile
/usr/bin/time -v -o ../kma_lto_O3_compile.txt make
...
do
    ...
    /usr/bin/time -v -o kma_lto_time_O3_1GB.txt -a kma -t 1 -int /dev/shm/fastp_output_trim.fastq -o /dev/shm/kma_output/output -t_db ~/main_folder/databases/resfinder_db/ResFinder -1t1 -ef -cge -nf -sam 4 > /dev/shm/output.sam
