...
if __name__ == "__main__":
    ...
    for file_id, ram_amount, minutes in zip(id_list, ram_list, minutes_list):
        ...
        program_command += f"touch {time_output_directory}/fastp_pipe_record_mem_{file_id}.out" + "\n"
        program_command += f"touch {time_output_directory}/kma_pipe_record_mem_{file_id}.out" + "\n"
        program_command += f"touch {time_output_directory}/samtools_sort_record_mem_{file_id}.out" + "\n"
        program_command += f"touch {time_output_directory}/bcftools_mpileup_record_mem_{file_id}.out" + "\n"
        program_command += f"touch {time_output_directory}/bcftools_call_record_mem_{file_id}.out" + "\n"
        program_command += f"touch {time_output_directory}/bcftools_norm_record_mem_{file_id}.out" + "\n"
        program_command += f"touch {time_output_directory}/bcftools_filter_record_mem_{file_id}.out" + "\n"
        program_command += f"cp -t /dev/shm/ ~/main_folder/data/raw/{file_id}_1.fastq.gz ~/main_folder/data/raw/{file_id}_2.fastq.gz" + "\n"



        program_command += f"mkdir /dev/shm/kma_output_mem"  + "\n"
        program_command += f"perf record -e cycles,instructions -g -o {time_output_directory}/fastp_pipe_record_mem_{file_id}.out fastp -w 1 -i /dev/shm/{file_id}_1.fastq.gz -I /dev/shm/{file_id}_2.fastq.gz --stdout "
        program_command += f"| perf record -e cycles,instructions -g -o {time_output_directory}/kma_pipe_record_mem_{file_id}.out kma -t 1 -int -- -o /dev/shm/kma_output_mem/output_{file_id} -t_db ~/main_folder/databases/resfinder_db/ResFinder -1t1 -ef -cge -nf -sam 4 "
        program_command += f"| samtools view -u - | perf record -e cycles,instructions -g -o {time_output_directory}/samtools_sort_record_mem_{file_id}.out samtools sort -@ 1 -u -o - - "
        program_command += f"| bash -c 'tee >(samtools view -bS - > /dev/shm/output_sorted_mem_{file_id}.bam)' "
        program_command += f"| perf record -e cycles,instructions -g -o {time_output_directory}/bcftools_mpileup_record_mem_{file_id}.out bcftools mpileup - --threads 1  -A -Ou -f ~/main_folder/databases/resfinder_db/all.fsa "
        program_command += f"| perf record -e cycles,instructions -g -o {time_output_directory}/bcftools_call_record_mem_{file_id}.out bcftools call - --ploidy 1 --threads 1 -mv -Ou "
        program_command += f"| perf record -e cycles,instructions -g -o {time_output_directory}/bcftools_norm_record_mem_{file_id}.out bcftools norm - -f ~/main_folder/databases/resfinder_db/all.fsa -Ou "
        program_command += f"| perf record -e cycles,instructions -g -o {time_output_directory}/bcftools_filter_record_mem_{file_id}.out bcftools view - -i 'QUAL>=20' -Oz -o /dev/shm/output_mem_{file_id}.vcf.gz ; "
        ...
        program_command += f"rm \"/dev/shm/{file_id}_1.fastq.gz /dev/shm/{file_id}_2.fastq.gz\"" + "\n"
        ...
        jobid = bsub_submit(cores=1, hosts=1, ram=ram_amount, exceed_mem=ram_amount,
                            ...
                            queue="hpc", name=f"pipe_stat_mem_{file_id}", \
                            output=f"{time_output_directory}/err_and_out/pipe_record_mem_{file_id}.out", \
                            error=f"{time_output_directory}/err_and_out/pipe_record_mem_{file_id}.err", \
