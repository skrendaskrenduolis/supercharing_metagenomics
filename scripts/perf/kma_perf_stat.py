...
if __name__ == "__main__":
    ...
    for file_id, ram_amount, minutes in zip(id_list, ram_list, minutes_list):
        ...
        # decompress file, pipe to fastp to make kma input file in memory
        if file_id == "1.8GB":
        ...
        # record file size
        program_command += f"touch {time_output_directory}/kma_performance_{file_id}.txt" + "\n"
        program_command += f"mkdir /dev/shm/kma_output" + "\n"

        # perf stat kma
        if file_id == "1.8GB": 
            program_command += f"for i in {{1..20}}; do\
                perf stat -o {time_output_directory}/kma_performance_{file_id}.txt --append\
                    kma -t 1 -int -i /dev/shm/{file_id}.trim.fastq -o /dev/shm/kma_output/{file_id}_output -t_db ../databases/resfinder_db/ResFinder -1t1 -ef -cge -nf -sam 4 > /dev/shm/{file_id}.output.sam ;\
                        rm \"/dev/shm/{file_id}.output.sam\" ; done" + "\n"
        else:
            program_command += f"for i in {{1..20}}; do\
                perf stat -o {time_output_directory}/kma_performance_{file_id}.txt --append\
                    kma -t 1 -i /dev/shm/{file_id}.trim.fastq -o /dev/shm/kma_output/{file_id}_output -t_db ../databases/resfinder_db/ResFinder -1t1 -ef -cge -nf -sam 4 > /dev/shm/{file_id}.output.sam ;\
                        rm \"/dev/shm/{file_id}.output.sam\" ; done" + "\n"
        ...
        jobid = bsub_submit(cores=1, hosts=1, ram=ram_amount, exceed_mem=ram_amount,
                            ...
                            queue="hpc", name=f"kma_stat_{file_id}", \
                            output=f"{time_output_directory}/err_and_out/kma_stat_{file_id}.out", \
                            error=f"{time_output_directory}/err_and_out/kma_stat_{file_id}.err", \
