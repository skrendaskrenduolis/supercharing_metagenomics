...
def bsub_submit(command='', directory='', modules='', runtime=60, cores=1, hosts=1, ram=1, queue='hpc', name='',
    ...
    # The submit
    # return script
    file_name = f'{directory}/{script_file_id}_kma_stat_temp.pbs.sh'
...
if __name__ == "__main__":
    ...
    for file_id, ram_amount, minutes in zip(id_list, ram_list, minutes_list):
        ...
        program_command += f"perf stat -o {time_output_directory}/fastp_pipe_stat_{file_id}.txt --append fastp -w 1 -i ~/main_folder/data/raw/{file_id}.fastq.gz --stdout "
        program_command += f"| perf stat -o {time_output_directory}/kma_pipe_stat_{file_id}.txt --append kma -t 1 -i -- -o /dev/shm/kma_output/output_{file_id} -t_db ~/main_folder/databases/resfinder_db/ResFinder -1t1 -ef -cge -nf -sam 4 "
        program_command += f"| samtools view -u - | perf stat -o {time_output_directory}/samtools_sort_stat_{file_id}.txt --append samtools sort -@ 1 -u -o - - "
        program_command += f"| bash -c 'tee >(samtools view -bS - > /dev/shm/output_sorted_{file_id}.bam)' "
        program_command += f"| perf stat -o {time_output_directory}/bcftools_mpileup_stat_{file_id}.txt --append bcftools mpileup - --threads 1  -A -Ou -f ~/main_folder/databases/resfinder_db/all.fsa "
        program_command += f"| perf stat -o {time_output_directory}/bcftools_call_stat_{file_id}.txt --append bcftools call - --ploidy 1 --threads 1 -mv -Ou "
        program_command += f"| perf stat -o {time_output_directory}/bcftools_norm_stat_{file_id}.txt --append bcftools norm - -f ~/main_folder/databases/resfinder_db/all.fsa -Ou "
        program_command += f"| perf stat -o {time_output_directory}/bcftools_filter_stat_{file_id}.txt --append bcftools view - -i 'QUAL>=20' -Oz -o /dev/shm/output_{file_id}.vcf.gz ; "
        ...
        jobid = bsub_submit(cores=1, hosts=1, ram=ram_amount, exceed_mem=ram_amount,
                            ...
                            output=f"{time_output_directory}/err_and_out/pipe_stat_{file_id}.out", \
