...
def bsub_submit(command='', directory='', modules='', runtime=60, cores=1, hosts=1, ram=1, queue='hpc', name='',
    ...
    # The submit
    # return script
    file_name = f'{directory}/{script_file_id}_pipe_stall_temp.pbs.sh'
...
if __name__ == "__main__":
    ...
    for file_id, ram_amount, minutes in zip(id_list, ram_list, minutes_list):
        ...
        #program_command += f"touch {time_output_directory}/fastp_pipe_stalls_record_{file_id}.out" + "\n"
        #program_command += f"touch {time_output_directory}/kma_pipe_stalls_record_{file_id}.out" + "\n"


        # run pipe once to record fastp
        program_command += f"mkdir /dev/shm/kma_output" + "\n"
        program_command += f"perf record -e cycles -e resource_stalls.any -e cycle_activity.stalls_total -g -o {time_output_directory}/fastp_pipe_stalls_record_{file_id}.out fastp -w 1 -i ~/main_folder/data/raw/{file_id}_1.fastq.gz -I ~/main_folder/data/raw/{file_id}_2.fastq.gz --stdout "
        program_command += f"| kma -t 1 -int -- -o /dev/shm/kma_output/output_{file_id} -t_db ~/main_folder/databases/resfinder_db/ResFinder -1t1 -ef -cge -nf -sam 4 > /dev/shm/output.sam" + "\n"
  
        # remove files from memory
        program_command += f"rm -r \"/dev/shm/kma_output\"" + "\n"
        program_command += f"rm \"/dev/shm/output.sam\"" + "\n"
        
        # run pipe again to record kma
        program_command += f"mkdir /dev/shm/kma_output" + "\n"
        program_command += f"fastp -w 1 -i ~/main_folder/data/raw/{file_id}_1.fastq.gz -I ~/main_folder/data/raw/{file_id}_2.fastq.gz --stdout "
        program_command += f"| perf record -e cycles -e resource_stalls.any -e cycle_activity.stalls_total -g -o {time_output_directory}/kma_pipe_stalls_record_{file_id}.out kma -t 1 -int -- -o /dev/shm/kma_output/output_{file_id} -t_db ~/main_folder/databases/resfinder_db/ResFinder -1t1 -ef -cge -nf -sam 4 > /dev/shm/output.sam" + "\n"
        ...
        jobid = bsub_submit(cores=1, hosts=1, ram=ram_amount, exceed_mem=ram_amount,
                            ...
                            queue="hpc", name=f"pipe_stall_record_{file_id}", \
                            output=f"{time_output_directory}/err_and_out/pipe_stall_record_{file_id}.out", \
                            error=f"{time_output_directory}/err_and_out/pipe_stall_record_{file_id}.err", \
